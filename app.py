...
def search():
    ...
    if q:
         articles=article.query.filter(article.title.contains(q) | article.body.contains(q))
    else:
...
def login():
    ...
    # (decoration) Check if user logged in 'me tet3ada ledashboard ela me tkount 3amel login'
def is_logged_in(f):
    @wraps(f)
